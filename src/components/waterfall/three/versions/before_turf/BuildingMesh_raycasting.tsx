...
export default function BuildingMesh({
  ...
  const gridVisualization = useMemo(() => {
    ...
    // Helper function to calculate grid square coverage (optimized)
    // NOTE: This helper expects GIS coordinates (x, y), not Three.js coordinates
    const calculateSquareCoverage = (gisMinX: number, gisMaxX: number, gisMinY: number, gisMaxY: number): number => {
      // Reduce samples from 5x5 to 2x2 for 6.25x performance improvement
      const samples = 2; // 2x2 grid of sample points (4 corners)
      let insideCount = 0;
      const totalSamples = samples * samples;

      for (let i = 0; i < samples; i++) {
        for (let j = 0; j < samples; j++) {
          const gisX = gisMinX + (gisMaxX - gisMinX) * (i + 0.5) / samples;
          const gisY = gisMinY + (gisMaxY - gisMinY) * (j + 0.5) / samples;
        ...
        for (let i = 0; i < linesA.length - 1 && !tileGenerationComplete; i++) {
          for (let j = 0; j < linesB.length - 1 && !tileGenerationComplete; j++) {
            ...
            if (corner1 && corner2 && corner3 && corner4) {
              ...
              // PERFORMANCE FIX: Use optimized 2x2 sampling (4 points)
              const samples = 2; // 2x2 grid of sample points
              let insideCount = 0;
              const totalSamples = samples * samples;
...
`);
    ...
    // 🧪 DIAGNOSTIC TESTS FOR IN/OUT CHECK
    // Test 1: First vertex should be inside polygon
    const testPoint1 = [footprint[0][0], footprint[0][1]];
    const isInside1 = isPointInsideFootprint(testPoint1[0], testPoint1[1]);
    console.log(`🧪 Test 1 - First vertex [${testPoint1[0].toFixed(2)}, ${testPoint1[1].toFixed(2)}]: isInside=${isInside1} (expected: TRUE)`);

    // Test 2: Center of polygon should be inside
    const centerX = footprint.reduce((sum, p) => sum + p[0], 0) / footprint.length;
    const centerY = footprint.reduce((sum, p) => sum + p[1], 0) / footprint.length;
    const centerInside = isPointInsideFootprint(centerX, centerY);
    console.log(`🧪 Test 2 - Polygon center [${centerX.toFixed(2)}, ${centerY.toFixed(2)}]: isInside=${centerInside} (expected: TRUE)`);

    // Test 3: Point far outside should be outside
    const outsidePoint = [minX - 100, minY - 100];
    const isOutside = isPointInsideFootprint(outsidePoint[0], outsidePoint[1]);
    console.log(`🧪 Test 3 - Far outside point [${outsidePoint[0].toFixed(2)}, ${outsidePoint[1].toFixed(2)}]: isInside=${isOutside} (expected: FALSE)`);

    // Test 4: Check polygon winding order (CCW = positive, CW = negative)
    const getSignedArea = (polygon: [number, number][]) => {
      let area = 0;
    ...
    console.log(`🧪 Test 4 - Polygon winding: ${signedArea > 0 ? 'CCW (Counter-Clockwise)' : 'CW (Clockwise)'}, signed area: ${signedArea.toFixed(2)}`);
